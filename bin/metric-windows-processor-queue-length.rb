...
class ProcessorQueueLengthMetric < Sensu::Plugin::Metric::CLI::Graphite
  option :scheme,
         description: 'Metric naming scheme, text to prepend to .$parent.$child',
         long: '--scheme SCHEME',
         default: Socket.gethostname.to_s

  def accquire_Processor_Queue_Length
    temp_arr = []
  ...
  def run
    values = accquire_Processor_Queue_Length
    metrics = {
      ActiveDirectory: {
        ProcessorQueueLength: values[0]
      }
    }
